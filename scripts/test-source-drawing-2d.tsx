...
const planHtml = renderToStaticMarkup(<SourceDrawing2D entities={planEntities} />);
...
const elevHtml = renderToStaticMarkup(<SourceDrawing2D entities={elevationEntities} />);
...
const combinedHtml = renderToStaticMarkup(<SourceDrawing2D entities={[...planEntities, ...elevationEntities]} />);
check("combined fixture (plan + elevation both present) shows the Plan/Elevation toggle", combinedHtml.includes("Elevation</button>") && combinedHtml.includes("Plan</button>"));
check("combined fixture defaults to elevation mode", combinedHtml.includes("<path") && !combinedHtml.includes("<line"));

const emptyHtml = renderToStaticMarkup(<SourceDrawing2D entities={[]} />);
